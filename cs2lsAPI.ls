...
app.post '/api/v1/users', (Q, res, next) ->
	if typeof Q.body == 'undefined' or Q.body == null
		return next()
	if typeof Q.body.email == 'undefined'
		winston.error 'No email'
		return next()
	if typeof Q.body.pass == 'undefined'
	...
	db_helpers.findUserByEmail email, (err, user) ->
		if err
			...
			return next()
		if typeof user != 'undefined' and user != null
			# already exists
...
app.post '/api/v1/users/:shortId/validation', (Q, res, next) ->
	if typeof Q.params.shortId == 'undefined'
		winston.error 'No shortId'
		return next()
	if typeof Q.query.sig == 'undefined'
	...
	db.UserModel.findByShortId shortId, (err, users) ->
		if err
			winston.error 'Error: ' + err
			return next()
		if typeof users == 'undefined' or !users.length
		...
		if user.validated
			...
			return next()
		# 3 - validate
		if Q.query.sig != user.validationSig
...
app.post '/api/v1/users/:email/reset_password_request', (Q, res, next) ->
	...
	if typeof Q.params.email == 'undefined'
		winston.error 'No email'
		return res.send('OK')
	# 1 - get user
...
app.put '/api/v1/users/:shortId/password', (Q, res, next) ->
	...
	if typeof Q.query.new_val == 'undefined'
	...
	if !helpers.validateShortId(shortId)
		winston.error 'Bad shortId'
		return next()
	if !helpers.validatePass(Q.query.new_val)
		winston.error 'Bad pass'
		return next()
	db.UserModel.findByShortId shortId, (err, users) ->
		if err
		...
		if !user.validated
			...
			return next()
		if typeof user.resetSig == 'undefined' or !user.resetSig.length
			winston.error 'No signature: ' + shortId
			return next()
		# 3 - validate
		if Q.query.sig != user.resetSig
			winston.error 'Can not validate user: ' + shortId
			return next()
		# 4 - set new password
		user.modified = Date.now()
